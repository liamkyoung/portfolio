import React from 'react'

function MainContent () {
  return (
    <div className='p-32'>
      <div className='flex justify-center relative z-200 mb-16'>
        <h1 className='font-notable text-5xl'>ABOUT ME</h1>
      </div>
      <div className='mx-auto my-0 font-paytone'>
        <h1> I fell in Love with computers AT THE AGE OF 5.</h1>
        <h1>I Am Excited to Build a career where I can make</h1>
        <h1>A DIFFERENCE Around the world with my code.</h1>
        <h1> Currently a Junior at The University OF Florida</h1>
        <h1>WITH AN INTEREST IN SOFTWARE ENGINEERING.</h1>
        <h1>TRYING TO FIND THE PERFECT INTERSECTION BETWEEN DESIGN, PROGRAMMING, CREATIVITY, and COLLABORATION.</h1>
        <h1>IF YOU WANT TO TALK TO A DETAIL-ORIENTED and EAGER DEVELOPER, PLEASE SEND ME A MESSAGE! I’d LOVE TO TALK.</h1>
      </div>
    </div>
  )
}

export default MainContent
